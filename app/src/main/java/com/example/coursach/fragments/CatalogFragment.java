...
public class CatalogFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View v, @Nullable Bundle s) {
        ...
        adapter = new ServiceAdapter(requireContext(), new ArrayList<>(), svc -> {
            ...
            i.putExtra("service_id", svc.getId());
            startActivity(i);
        });
        ...
        chipGroup.setOnCheckedStateChangeListener((g, ids) -> {
            if (!ids.isEmpty()) {
                Chip c2 = g.findViewById(ids.get(0));
                if (c2 != null) {
                    selCat = c2.getText().toString().equals("Все") ? "" : c2.getText().toString();
                    filter();
                }
            }
        });
    ...
    public void load() {
        ...
        Log.d(TAG, "Starting service load from Supabase");
        exec.execute(() -> {
            try {
                ...
                r.close();
                Log.d(TAG, "Response received: ok=" + ok + ", length=" + body.length());
                List<Service> list = new ArrayList<>();
                if (ok) {
                    JSONArray arr = new JSONArray(body);
                    Log.d(TAG, "Parsing " + arr.length() + " services");
                ...
                ui(() -> {
                    ...
                    filter();
                    Log.d(TAG, "UI updated: " + list.size() + " items displayed");
                });
            } catch (Exception e) {
